...
function streamTrack(streamUrl) {
	console.log("Trying to stream: " + streamUrl);

	get(streamUrl).then(function (response) {
		var fileWriteStream = fs.createWriteStream(path.join(__dirname, '..', '/tmp/test.mp3'), {
				flags: 'w+'	
		...
		var fileReadStream = fs.createReadStream(path.join(__dirname, '..', '/tmp/test.mp3'));
